...
const addProduto = async (request, response) => {
  const clienteId = request.params.clienterId
...
//   produto.nivel = calcularPreco(request.body.inicio, request.body.fim, produto.nivel)

//   return cliente.save((error) => {
//     if (error) {
//       return response.status(500).send(error)
//     }
...
const getProdutos = async (request, response) => {
  ...
  await clientesModel.findById(clienteId, (error, cliente) => {
    if (error) {
      return response.status(500).send(error)
    }

    if (cliente) {
      return response.status(200).send(cliente.produtos)
    }
...
const getProdutoById = async (request, response) => {
  const clienteId = request.params.clienteId
  const produtoId = request.params.produtoId
  const cliente = await clientesModel.findById(clienteId)
  const produto = cliente.produtos.find(produto => produto._id == produtoId)
